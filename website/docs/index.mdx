...
sidebar:
  label: Introduction
title: VitalRouter
description: VitalRouter, is a source-generator powered zero-allocation fast in-memory messaging library for Unity and .NET.
---
...
- **Optional Extensions**
  ...
  - [MRuby scripting!](./extensions/mruby/intro)
...
// lambda interceptors
//
// `WithFilter(...)` returns a derived child router that owns the filter.
// Subscribers on the child receive commands published on the parent with
// the filter applied (Rx `Where`-style chaining).
router
...
<div style={{ width: '50%' }}>
<ThemedImage light="/img/diagrams/diagram_interceptors.svg" dark="/img/diagrams/diagram_interceptors_dark.svg" alt="Async interceptor stack" />
</div>

## Sequential Control

One of VitalRouter's standout features is **declarative control over how concurrent async handlers run**.
This matters most in games, where dialogue, cutscenes, and tutorials must play *in order* — never overlapping.

Take a cutscene. You just publish commands, and VitalRouter plays them back-to-back.

[Read more](./pipeline/sequential-control)
...
You can also choose other strategies — `Drop` to ignore overlapping commands (debounce), or `Switch` to cancel the running handler and let the latest one win.
...
![MRuby and C# Diagram](./assets/diagram_mruby.png)

[Read more](./extensions/mruby/intro)
...
|-------------------------- |----------:|---------:|---------:|-------:|----------:|
| 'Publish (ZeroMessenger)' |  26.63 ns | 0.560 ns | 2.114 ns |      - |         - |
| 'Publish (VitalRouter)'   |  27.01 ns | 0.439 ns | 0.877 ns |      - |         - |
| 'Publish (MessagePipe)'   |  36.06 ns | 0.531 ns | 1.023 ns | 0.0048 |      40 B |
| 'Publish (MediatR)'       |  39.70 ns | 0.547 ns | 0.867 ns | 0.0076 |      64 B |
| 'Publish (PubSub)'        | 102.57 ns | 1.450 ns | 2.687 ns | 0.0229 |     192 B |
